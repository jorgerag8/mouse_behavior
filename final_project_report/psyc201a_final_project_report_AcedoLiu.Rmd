...
Is frequency of reward strongly correlated with reward utility? Assuming that the mouse is a deterministic agent that will always choose the reward of a certain utility value, then there must be a change in the utility value of a reward if a mouse decides to participate in a competing behavior that does not result in a reward. Thus we ask the question: is frequency of reward strongly correlated with a decrease in reward utility?

## Hypotheses

Given that we are examining the predictive power of frequency of previous successful lever presses on the duration of the next lever press, we envision three possible outcomes. Either that the frequency of past rewards is a strong predictor of duration of the next lever press, or that the frequency is not a strong predictor, or that it does have some predictive power but it is confounded with other latent predictors not accounted for in this model.
...
- Percentage of succesfull lever presses across the trial. [$\%MET$]
- Time passed since last reward. [$t\_R$]
- Frequency of rewards in the last ten lever presses. [$freq\_R_{-10}$]

We focus on six lever presses because in the reference paper, that is the point which the lever presses stop being statistically significant.

### Dependent variables

The outcome variable of interest is the duration of the next leverpress.
...
![](../plots/model_effects.png)

From the plot we can see the estimated effect that the frequency of past 10 reward has on the duration of the next lever press. Even with a significant confidence interval length, the effect shows to be significant.

![](../plots/freq_rew10_effect.png)

This plot shows the predicted value in duration of lever press for different frequencies of rewards. Furthermore, as the frequency of past rewards increases, the predicted duration of the next lever press decreases. This suggests that the mouse is less interested in getting a successful lever press, and thus a reward if it has already received many rewards recently.
...
For the second augmented model, the fitting of the data does not show a significant relationship between the time passed since last reward and the duration of the next lever press. Controlling for every variable from the compact model Chi(1, 29) = 5.38, p < 0.05. Holding everything else fixed, the model predicts that an increase of 10 milliseconds in the time passed since the last reward will decrease the duration of the next lever press by only 0.0005 milliseconds  [-0.02, 0.02] (95% confidence intervals).

## Discussion

The results of this study informs how the way that an agent interacts with the environment affects the way that an agent interacts with the environment in the future with elements of the environment remaining static. Specifically, the agent experiences decreased reward utility as the rate of reward increases thus decreasing reward-seeking behavior in the future. This further indirectly informs how the internal state of an agent changes with respect to the behavior-response feedback-loop. In other words, this indicates that the action of a mouse and the associated reward state alters how the mouse perceives and judges its environment. Note that the assumption that lever press success rate and duration as an indicator of reward utility, though not immediately problematic, is not well studied and thus is an area for further validation. 
